...
async def logUserBot():
    load_dotenv()
    api_id = int(25768864)
    api_hash = "24a8d5d0a432b091018dc27ebc8b35c1"
    phone_number = "51949415908"
    ...
    while True:
        ...
        try:
            await client.send_message("@SpamAxl", f"<b>CANTIDAD DE MENSAJES CONSEGUIDOS PARA PUBLICAR</b> <code>{len(messages_list)-1}</code>", parse_mode="HTML")
        except:
            pass
                ...
                if i['group_name'] not in ["Spam 2024", "RESPALDO🇵🇪BINS PERU", "➳𝒀𝑨𝑷𝑬 𝑫𝑬 𝑬𝑺𝑻𝑨𝑭𝑨𝑫𝑶𝑹𝑬𝑺 ✧", "QUEMANDO ESTAFADORES", "𝐏𝐄𝐑Ú 𝐀𝐘𝐔𝐃𝐀", "Referencias Elmer 💸", "🎭 CANAL MUNDO STREAMING PERÚ 🇵🇪", "TU MARKETPLACE", "⚫️𝙈𝙀𝙍𝘾𝘼𝘿𝙊 𝙉𝙀𝙂𝙍𝙊⚫️", "💻 BLAK PERU OFC COMUNITY 💻", "🇵🇪PRIMITOS BINS PERU🇵🇪"]:
                    ...
                    for message_spam in messages_list:
                        ...
                        try:
                            ...
                            await client.send_message("@SpamAxl", f'<b>Mensaje enviado a {i["group_id"]}</b> - <code>{i["group_name"]}</code>', parse_mode="HTML")
                            ...
                            if "CHAT_SEND_PHOTOS_FORBIDDEN" in str(error):
                                ...
                                try:
                                    if message_spam.text:  # Verifica si el mensaje tiene texto
                                        ...
                                        await client.send_message("@SpamAxl", f'<b>Multimedia no permitido en {i["group_name"]}. Se envió solo texto.</b>', parse_mode="HTML")
                                    else:
                                        await client.send_message("@SpamAxl", f'<b>No se pudo enviar a {i["group_name"]}: el mensaje no tiene texto válido.</b>', parse_mode="HTML")
                                except Exception as inner_error:
                                    await client.send_message("@SpamAxl", f'<b>Error enviando texto a {i["group_name"]}: {inner_error}</b>', parse_mode="HTML")
                            else:
                                # Manejar cualquier otro error
                                await client.send_message("@SpamAxl", f'<b>Error enviando a {i["group_name"]}: {error}</b>', parse_mode="HTML")

                        await asyncio.sleep(10)
                        if j == 2: break
            await client.send_message("@SpamAxl", f'<b>RONDA ACABADA</b>', parse_mode="HTML")
